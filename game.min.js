...
else{if(entity.pos.y<ig.game.screen.y*1||entity.pos.y>(ig.game.screen.y+ig.system.height)*1||entity.pos.x<ig.game.screen.x*1||entity.pos.x>(ig.game.screen.x+ig.system.width)*1){entity.kill();}}},bounceBackOffScreen:function(entity){if(entity.pos.x<0||entity.pos.x+entity.size.x>ig.system.width){entity.vel.x=-entity.vel.x;}
if(entity.pos.y<0||entity.pos.y+entity.size.y>ig.system.height){entity.vel.y=-entity.vel.y;}},calcAngle:function(entity,angle){if(angle){var sinAngle=Math.sin(angle);var cosAngle=Math.cos(angle);}
else{var sinAngle=Math.sin(entity.angle);var cosAngle=Math.cos(entity.angle);}
var bulletX=(entity.pos.x+entity.size.x/2)+(sinAngle);var bulletY=(entity.pos.y+entity.size.y/2)-(cosAngle);return{x:bulletX,y:bulletY,sin:sinAngle,cos:cosAngle};},splitLines:function(text,font,maxTextWidth){var words=text.split(' ');var lastWord=words[words.length-1];var lineWidth=0;var wordWidth=0;var thisLine='';var allLines=[];ig.game.ctx.save();ig.game.ctx.font=font;for(var i=0;i<words.length;i++){var word=words[i];thisLine=thisLine.concat(word+' ');lineWidth=ig.game.ctx.measureText(thisLine).width;if(word!==lastWord){var nextWord=words[i+1];if(lineWidth+ig.game.ctx.measureText(nextWord).width>=maxTextWidth){addToAllLines(thisLine);}
else if(word==='~'){addToAllLines(' ');}
else if(nextWord==='~'){addToAllLines(thisLine);}}
else{addToAllLines(thisLine);return allLines;}
ig.game.ctx.restore();}
function addToAllLines(text){allLines.push(text);thisLine='';lineWidth=0;}},randomFromTo:function(from,to){return Math.floor(Math.random()*(to-from+1)+from);},toDegrees:function(num){return num*(180/Math.PI);},toRadians:function(num){return num*(Math.PI/180);},removeFromArray:function(object,array){var index=array.indexOf(object);array.splice(index,1);},isOdd:function(number){return(number%2)==1;},pickRandomColor:function(mode){var color={};switch(mode){case'hsl':var newTargetH=this.randomFromTo(0,360);while(newTargetH<targetColor.h+10&&newTargetH>targetColor.h-10){newTargetH=this.randomFromTo(0,360);}
color.h=newTargetH;color.s=this.randomFromTo(20,100);color.l=this.randomFromTo(20,70);return color;case'rgb':color.r=this.randomFromTo(0,255);color.g=this.randomFromTo(0,255);color.b=this.randomFromTo(0,255);return color;case'hex':default:return'#'+Math.floor(Math.random()*16777215).toString(16);}},shuffleArr:function(array){for(var j,x,i=array.length;i;j=Math.floor(Math.random()*i),x=array[--i],array[i]=array[j],array[j]=x);return array;},pause:function(){ig.Timer.timeScale=(ig.Timer.timeScale===0?1:0);this._paused=ig.Timer.timeScale===0;ig.game.paused=!(ig.game.paused);}});});

// lib/game/entities/circle.js
ig.baked=true;ig.module('game.entities.circle').requires('impact.entity').defines(function(){EntityCircle=ig.Entity.extend({type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.B,collides:ig.Entity.COLLIDES.ACTIVE,clickable:true,size:{x:30,y:30},speed:0.01,color:'#ffffff',alpha:0.5,highlightedTimer:new ig.Timer(1),sound:null,init:function(x,y,settings){this.parent(x,y,settings);this.size.x=ig.game.controller.randomFromTo(ig.game.minCircleSize,ig.game.maxCircleSize);this.size.y=this.size.x;this.radius=this.size.x-(20*this.size.x/100);this.speed=ig.game.controller.randomFromTo(ig.game.minCircleSpeed,ig.game.maxCircleSpeed);this.pickPosition();this.color=ig.game.controller.pickRandomColor('rgb');this.ctx=ig.system.context;this.vel.x=this.speed;this.vel.y=this.speed;var rand=ig.game.controller.randomFromTo(0,ig.game.soundsArr.length-1);this.sound=ig.game.soundsArr[rand];},pickPosition:function(){this.pos.x=ig.game.controller.randomFromTo(0+this.size.x,ig.system.width-this.size.x);this.pos.y=ig.game.controller.randomFromTo(0+this.size.y,ig.system.height-this.size.y);var allCircles=ig.game.getEntitiesByType(EntityCircle);for(var i=0;i<allCircles.length;i++){var circle=allCircles[i];if(circle!==this){if(this.touches(circle)){this.pickPosition();}}}},highlight:function(time){this.highlightedTimer.set(time);this.sound.play();this.alpha=1;this.highlighted=true;},update:function(){ig.game.controller.bounceBackOffScreen(this);if(this.highlighted&&this.highlightedTimer.delta()>0){this.alpha=0.5;this.highlighted=false;}
this.parent();},kill:function(){this.parent();},clicked:function(){console.log('clicked');if(this===ig.game.sequenceArr[ig.game.clickCount-1]){console.log('right!');this.sound.play();if(ig.game.clickCount===ig.game.sequenceArr.length){ig.game.increaseDifficulty();}}
else{ig.game.lives--;if(ig.game.lives>0){ig.game.replaySequence();ig.game.fail.play();}
else{ig.game.restartGame();ig.game.bigfail.play();}}},draw:function(){this.ctx.save();this.ctx.fillStyle='rgba('+this.color.r+','+this.color.g+','+this.color.b+','+this.alpha+')';this.ctx.beginPath();this.ctx.arc(this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,this.radius,0,2*Math.PI);if(this.highlighted){this.ctx.shadowBlur=50;this.ctx.shadowColor='rgba('+this.color.r+','+this.color.g+','+this.color.b+','+this.alpha+')';}
this.ctx.fill();this.ctx.restore();this.parent();}});});

// lib/game/entities/pointer.js
ig.baked=true;ig.module('game.entities.pointer').requires('impact.entity').defines(function(){EntityPointer=ig.Entity.extend({type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,zIndex:100,size:{x:1,y:1},radius:5,mousePressed:{x:0,y:0},holdingEntity:null,touchingEntity:null,activeColor:'green',inactiveColor:'red',active:false,init:function(x,y,settings){this.parent(x,y,settings);this.ctx=ig.system.context;ig.game.pointer=this;},update:function(){this.pos.x=ig.input.mouse.x;this.pos.y=ig.input.mouse.y;if(ig.game.sequenceDisplayTimer&&this.active){this.active=false;if(ig.game.clickCount>0){ig.game.clickCount=0;}}
else if(!ig.game.sequenceDisplayTimer&&!this.active){this.active=true;}
this.parent();},kill:function(){this.parent();},check:function(other){if(this.active&&ig.input.released('click')&&other.clickable){ig.game.clickCount++;other.clicked();}},draw:function(){this.parent();}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.misc.controller','game.entities.circle','game.entities.pointer').defines(function(){SimonSays=ig.Game.extend({font:new ig.Font('media/04b03.font.png'),controller:new ig.Controller(),maxCircleSize:25,minCircleSize:0,minCircleSpeed:-10,maxCircleSpeed:10,difficulty:1,minCircleQuantity:0,maxCircleQuantity:0,sequenceArr:[],highlightedCircle:0,selectedArr:[],clickCount:0,lives:3,select0:new ig.Sound('media/audio/select0.*',false),select1:new ig.Sound('media/audio/select1.*',false),select2:new ig.Sound('media/audio/select2.*',false),select3:new ig.Sound('media/audio/select3.*',false),select4:new ig.Sound('media/audio/select4.*',false),soundsArr:[],fail:new ig.Sound('media/audio/fail.*',false),bigfail:new ig.Sound('media/audio/bigfail.*',false),mode:'start',init:function(){this.sizeGame(window.innerWidth,window.innerHeight);this.ctx=ig.system.context;ig.input.bind(ig.KEY.MOUSE1,'click');this.spawnEntity(EntityPointer,0,0);this.soundsArr=[this.select0,this.select1,this.select2,this.select3,this.select4];this.startGame('start');},sizeGame:function(width,height){this.sequenceArr.length=0;this.clickCount=0;this.highlightedCircle=0;this.maxCircleSize=20*width/100;if(this.maxCircleSize<10){this.maxCircleSize=10;}
this.minCircleQuantity=this.difficulty+1;this.maxCircleQuantity=this.difficulty+3;},startGame:function(mode){this.mode=mode;var circleQuantity=this.controller.randomFromTo(this.minCircleQuantity,this.maxCircleQuantity);for(var i=0;i<circleQuantity;i++){this.spawnEntity(EntityCircle,0,0);}
switch(mode){case'start':this.selectCircles(this.difficulty);break;}},restartGame:function(){var allCircles=this.getEntitiesByType(EntityCircle);for(var i=0;i<allCircles.length;i++){var circle=allCircles[i];circle.kill();}
this.highlightedCircle=0;this.sequenceArr.length=0;this.selectedArr.length=0;this.difficulty=1;this.lives=3;this.startGame('start');},increaseDifficulty:function(){this.clickCount=0;this.highlightedCircle=0;this.difficulty++;this.sequenceArr.length=0;this.selectedArr.length=0;this.spawnEntity(EntityCircle,0,0);this.selectCircles(this.difficulty);},replaySequence:function(){this.clickCount=0;this.highlightedCircle=0;this.sequenceDisplayTimer=new ig.Timer(2);this.displayingSequence=true;},selectCircles:function(quantity){var shuffledArr=this.controller.shuffleArr(this.getEntitiesByType(EntityCircle));for(var i=0;i<quantity;i++){this.sequenceArr.push(shuffledArr[i]);}
this.displayingSequence=true;this.sequenceDisplayTimer=new ig.Timer(1);},update:function(){this.parent();if(this.displayingSequence){if(this.sequenceDisplayTimer.delta()>0){var circle=this.sequenceArr[this.highlightedCircle];this.sequenceDisplayTimer.reset();circle.highlight(1.5);if(circle===this.sequenceArr[this.sequenceArr.length-1]){this.displayingSequence=false;this.sequenceDisplayTimer=null;}
this.highlightedCircle+=1;}}},drawMenu:function(){this.ctx.save();var x=ig.system.width/2;var y=100;this.ctx.font="bold 100px Verdana";this.ctx.fillStyle="rgba(255,255,255,"+0.5+")";this.ctx.textAlign="center";this.ctx.fillText("SIMON SAYS",x,y);y+=100;this.ctx.font="30px Verdana";this.ctx.fillText("Click the floating circles in the displayed order",x,y);y+=30;this.ctx.fillText("Difficulty: "+this.difficulty,x,y);y+=30;this.ctx.fillText("Lives: "+this.lives,x,y);this.ctx.restore();},draw:function(){this.parent();switch(this.mode){case'start':this.drawMenu();break;}
this.ctx.save();if(this.pointer.active){this.ctx.fillStyle=this.pointer.activeColor;}
else{this.ctx.fillStyle=this.pointer.inactiveColor;}
this.ctx.beginPath();this.ctx.arc(this.pointer.pos.x+this.pointer.size.x/2,this.pointer.pos.y+this.pointer.size.y/2,this.pointer.radius,0,2*Math.PI);this.ctx.fill();this.ctx.restore();}});ig.main('#canvas',SimonSays,60,window.innerWidth,window.innerHeight,1);function resizeCanvas(){console.log('resizing');ig.system.resize(window.innerWidth,window.innerHeight,1);}});