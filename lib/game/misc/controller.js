...
ig.Controller = ig.Class.extend({
	...
	pickRandomColor: function(mode) {
		var color = {};
		switch (mode) {
			case 'hsl':
				// Select random hue
				var newTargetH = this.randomFromTo(0,360);
				...
				while (newTargetH < targetColor.h + 10 && newTargetH > targetColor.h - 10) {
					newTargetH = this.randomFromTo(0,360);
				}
				// Set targetColor.h to new hue.
				color.h = newTargetH;

				// Constrain saturation and lightness to produce more pleasing/interesting colors.
				color.s = this.randomFromTo(20,100);
				color.l = this.randomFromTo(20,70);
				return color;
			case 'rgb':
				color.r = this.randomFromTo(0,255);
				color.g = this.randomFromTo(0,255);
				color.b = this.randomFromTo(0,255);
				return color;
			case 'hex':
			default:
